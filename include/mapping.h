...
//	static unsigned bitmap_size;
	...
	mapping(unsigned sz, unsigned bitmap_size = 0, bool init = false)
	{ 
		ordering = new vertex_t[sz]; 

        if(bitmap_size == 0) bitmap = NULL;
        else bitmap = new unsigned long long[bitmap_size];

		if(init){
			size = 0;
			for(unsigned i = 0; i < sz; i++) ordering[i] = i;
            if(bitmap) memset(bitmap, 0, sizeof(unsigned long long)*bitmap_size);
		}
