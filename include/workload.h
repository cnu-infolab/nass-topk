...
#define THRESHOLD_MAX 6 // maximum threshold
...
public:
    ...
    void generateWorkload(vector< pair<int, int> >* nassIndex){
        loaded_from_a_custom_file = false;
        ...
        while(workload.size() < 100){
            ...
            int cnt = 0;
            for(unsigned i = 0; i < nassIndex[gid].size(); i++)
                if(nassIndex[gid][i].second < THRESHOLD_MAX + 1) cnt++;

            if(cnt > 30){
                bool contained = false;
                for(unsigned i = 0; i < workload.size(); i++)
                    if(g == workload[i]){ contained = true; break; }
                if(!contained) workload.push_back(g); 
            }
        }
    }
