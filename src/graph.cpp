...
// mapping without bitmap
mapping* graph::generateNextState(int next_vertex, mapping* pm)
{
    mapping* m = new mapping(src.vsize());
...
{
    bool local_mapping_info[src.vsize()];

	for(unsigned i = 0; i < src.vsize(); i++){
		if(i < border) local_mapping_info[ordering[i]] = true;
		else local_mapping_info[ordering[i]] = false;
	}
	...
	for(unsigned v = 0; v < vsize(); v++){
		...
		for(unsigned i = 0; i < adjacent[O(v)].size(); i++){
			short target = adjacent[O(v)][i];
			if(!local_mapping_info[target]) vertex_efreq[v][src(O(v), target) - 1]++;
		}
